...
func _on_room_changed(target_room: String):
	...
	if prev_room_instance:
		...
		player.position = Vector2(fposmod(player.position.x, MetSys.map_size.x * MetSys.settings.in_game_cell_size.x), fposmod(player.position.y, MetSys.map_size.y * MetSys.settings.in_game_cell_size.y))
